...
bool ServerMapGimmick::CsvRead(const char* filename)
{
	FILE*  fp = NULL;
...
void ServerMapGimmick::Init()
{
	for (int i = 0; i < MAP_HEIGHT; i++)
	{
		for (int j = 0; j < MAP_WIDTH; j++)
		{
			...
			{
			case GATE_01:
				m_Gate.push_back(ServerGate(m_pLibrary));
				break;
			case SWITCH_RED_01:
				int Id = m_GimmickData[i][j] % 10000;
				m_Switch.push_back(ServerSwitch(m_pLibrary, Id, m_GimmickData, SWITCH_RED_01));
				break;
...
void ServerMapGimmick::Control()
{

}


void ServerMapGimmick::Draw(float DrawPosX, float DrawPosY)
{
...
int ServerMapGimmick::GimmickCheck(float x, float y)
{
...
void ServerMapGimmick::SwitchOn(float x, float y)
{
	int arrayx = 0, arrayy = 0;
	int switchCount = 0;
	arrayx = int(x / 64);
...
void ServerMapGimmick::SwitchOnYoung(float x, float y)
{
	int arrayx = 0, arrayy = 0;
	int switchCount = 0;
	arrayx = int(x / 64);
...
void ServerMapGimmick::SwitchOff(float x, float y)
{
	int arrayx = 0, arrayy = 0;
	arrayx = int(x / 64);
	arrayy = int(y / 64);
...
void ServerMapGimmick::MapTex_UV_Set(float Posx, float Posy)
{
...
void ServerMapGimmick::MapTex_XY_Set(float Posx, float Posy)
{
	int count_x = int((Posx - 64) / MAPTIP_SIZE);
	int count_y = int((Posy - 64) / MAPTIP_SIZE);
...
bool ServerMapGimmick::HoleCheck(float x, float y)
{
	int arrayx = 0, arrayy = 0;
	arrayx = int(x / 64);
	arrayy = int(y / 64);
...
bool ServerMapGimmick::WoodBoxHoleCheck(float x, float y)
{
	int arrayx = 0, arrayy = 0;
	arrayx = int(x / 64);
	arrayy = int(y / 64);
...
bool ServerMapGimmick::AppleCheck(float x, float y)
{
	int arrayx = 0, arrayy = 0;
	arrayx = int(x / 64);
	arrayy = int(y / 64);
