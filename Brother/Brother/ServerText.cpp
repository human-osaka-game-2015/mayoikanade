...
ServerText::ServerText(Library* pLibrary, bool* pPadState, bool* pPadOldState, PADSTATE* pButtonState) :m_pLibrary(pLibrary), m_darkness_alpha(0), m_brother_alpha(0), m_textbox_alpha(0), m_message_count(0), m_message_old(0), m_send_count(0)
...
void ServerText::TextControl()
{
	...
	if (m_all_alpha_is_max)
	{
		...
		if (m_pLibrary->GetButtonState(GAMEPAD_A, GAMEPAD1) == PAD_PUSH&&m_message_wait == false && m_text_is_end == false)
		{
			m_message_count += 1;
			m_message_wait = true;
